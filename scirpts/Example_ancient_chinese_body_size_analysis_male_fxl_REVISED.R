...
required_packages <- c(
  ...
  "terra", "tidyterra", "RColorBrewer", "readxl", "ggnewscale"
)

auto_install <- FALSE

# ---- Reproducibility option ----
# FALSE: standard multi-core fitting
# TRUE:  single-core fitting with a fixed seed for reproducibility checking
reproducible_run <- FALSE
fit_cores <- if (reproducible_run) 1L else 4L
fit_seed  <- if (reproducible_run) 42L else NA_integer_
...
ancient_chi_detail <- ancient_chi_detail %>%
  drop_na(Mintemp, Maxtemp, Minprecip, Maxprecip, Altitude)

# ---- Primary analytical sample ----
# Sding-Chung is excluded from the primary analyses because femoral dimensions
# contributed to sex classification in this commingled assemblage.
# The full climate-complete dataset is retained for the sensitivity analysis.
is_sding_chung <- function(x) {
  x_norm <- x %>%
    as.character() %>%
    stringr::str_to_lower() %>%
    stringr::str_replace_all("[ _-]+", "")
  stringr::str_detect(x_norm, "sdingchung")
}

ancient_chi_detail_all <- ancient_chi_detail

ancient_chi_detail <- ancient_chi_detail_all %>%
  filter(!is_sding_chung(site_id))

message(
  "Primary dataset excludes ",
  sum(is_sding_chung(ancient_chi_detail_all$site_id), na.rm = TRUE),
  " Sding-Chung rows."
)

## Separate males and females in the primary analytical sample
...
# Trait-specific centring and scaling parameters are saved below after
# filtering to the analytical sample for each outcome. The same parameters
# are then reused for spatial prediction and sensitivity analyses.
...
## For FXL
### Male
#### Prepare the primary analytical data
# Trait-specific available-case analysis:
# retain individuals with FXL and the required temporal/environmental covariates.
# Missing skeletal measurements are not imputed.

ancient_chi_male_FXL_source <- ancient_chi_male %>%
  filter(!is.na(FXL), !is.na(Time_minus), !is.na(Time_plus))

# Save the exact centring/scaling parameters used for this model so that
# prediction grids and sensitivity data are transformed identically.
get_scale_par <- function(x) {
  c(center = mean(x, na.rm = TRUE), scale = sd(x, na.rm = TRUE))
}

male_FXL_scaling <- list(
  longitude = get_scale_par(ancient_chi_male_FXL_source$longitude),
  latitude  = get_scale_par(ancient_chi_male_FXL_source$latitude),
  Mintemp   = get_scale_par(ancient_chi_male_FXL_source$Mintemp),
  Maxtemp   = get_scale_par(ancient_chi_male_FXL_source$Maxtemp),
  Minprecip = get_scale_par(ancient_chi_male_FXL_source$Minprecip),
  Maxprecip = get_scale_par(ancient_chi_male_FXL_source$Maxprecip),
  Altitude  = get_scale_par(ancient_chi_male_FXL_source$Altitude),
  FXL       = get_scale_par(ancient_chi_male_FXL_source$FXL)
)

ancient_chi_male_FXL <- ancient_chi_male_FXL_source %>%
  mutate(
    longitude_scaled = (longitude - male_FXL_scaling$longitude["center"]) /
      male_FXL_scaling$longitude["scale"],
    latitude_scaled  = (latitude - male_FXL_scaling$latitude["center"]) /
      male_FXL_scaling$latitude["scale"],
    mintemp_scaled   = (Mintemp - male_FXL_scaling$Mintemp["center"]) /
      male_FXL_scaling$Mintemp["scale"],
    maxtemp_scaled   = (Maxtemp - male_FXL_scaling$Maxtemp["center"]) /
      male_FXL_scaling$Maxtemp["scale"],
    minprecip_scaled = (Minprecip - male_FXL_scaling$Minprecip["center"]) /
      male_FXL_scaling$Minprecip["scale"],
    maxprecip_scaled = (Maxprecip - male_FXL_scaling$Maxprecip["center"]) /
      male_FXL_scaling$Maxprecip["scale"],
    altitude_scaled  = (Altitude - male_FXL_scaling$Altitude["center"]) /
      male_FXL_scaling$Altitude["scale"],
    FXL_z            = (FXL - male_FXL_scaling$FXL["center"]) /
      male_FXL_scaling$FXL["scale"]
  )

mean_fxl_male <- male_FXL_scaling$FXL["center"]
sd_fxl_male   <- male_FXL_scaling$FXL["scale"]

mean_fxl_male
sd_fxl_male

#### Fit the primary model
# Internal period codes are retained for compatibility with the data file.
# Display labels below follow the revised manuscript terminology.
...
# Backward-compatible alias used by the plotting/prediction code below.
fit_male_FXL <- fit_male_FXL_main

#### Sensitivity analysis: include Sding-Chung
# Sding-Chung remains excluded from the primary model. This sensitivity model
# reintroduces it while using the *primary-model* scaling parameters so that
# posterior estimates remain directly comparable on the same standardised scale.

ancient_chi_male_FXL_sensitivity <- ancient_chi_detail_all %>%
  filter(
    sex == '0',
    !is.na(FXL),
    !is.na(Time_minus),
...
contrasts(ancient_chi_male_FXL_sensitivity$period) <- contr.sum(length(levs))

has_sding_chung_FXL <- any(
  is_sding_chung(ancient_chi_male_FXL_sensitivity$site_id),
  na.rm = TRUE
)

if (has_sding_chung_FXL) {
  fit_male_FXL_with_SDC <- brm(
    FXL_z ~
      period +
      t2(longitude_scaled, latitude_scaled) +
      mintemp_scaled + maxtemp_scaled +
      ...
      altitude_scaled +
      (1 | site_id),
    data    = ancient_chi_male_FXL_sensitivity,
    family  = student(),
    prior   = priors_common,
    iter    = 4000,
    warmup  = 1000,
  ...
  sensitivity_fixed_effects_male_FXL <- bind_rows(
    as.data.frame(fixef(fit_male_FXL_main, probs = c(0.05, 0.95))) %>%
      rownames_to_column("parameter") %>%
      mutate(model = "Primary: Sding-Chung excluded"),
    as.data.frame(fixef(fit_male_FXL_with_SDC, probs = c(0.05, 0.95))) %>%
      rownames_to_column("parameter") %>%
      mutate(model = "Sensitivity: Sding-Chung included")
  ) %>%
    relocate(model, parameter)

  print(sensitivity_fixed_effects_male_FXL)
} else {
  fit_male_FXL_with_SDC <- NULL
  sensitivity_fixed_effects_male_FXL <- NULL
  warning(
    "No Sding-Chung rows were found in the input data; ",
    "the sensitivity model was therefore not fitted."
  )
}

##### Primary-model summary: 90% equal-tailed CrI
levels(fit_male_FXL$data$period)
contrasts(fit_male_FXL$data$period)
summary(fit_male_FXL, prob = 0.90)
fixef(fit_male_FXL, probs = c(0.05, 0.95))
...
# Explicit 90% equal-tailed CrIs (5th and 95th posterior percentiles)
fixef_cri90_male_fxl <- fixef_long_male_fxl_ranked %>%
  group_by(parameter) %>%
  summarise(
    .point = median(.value),
...
p_fixed_male_fxl <- ggplot(fixef_long_male_fxl_ranked, aes(x = .value, y = parameter)) +
  ...
  geom_segment(
    data = fixef_cri90_male_fxl,
    aes(x = .lower, xend = .upper, y = parameter, yend = parameter),
    inherit.aes = FALSE,
    colour = "steelblue",
    linewidth = 0.8
  ) +
...
# 2) 90% equal-tailed CrI + median
site_cri90_male_FXL <- site_re_male_FXL %>%
  group_by(site_id) %>%
  median_qi(r_site_id, .width = 0.90) %>%
  ungroup() %>%
  transmute(
    site_id,
...
site_cri90_male_FXL <- site_cri90_male_FXL %>%
  left_join(site_point_male_FXL, by = "site_id") %>%
  mutate(
    deviated_90 = !(.lower <= 0 & .upper >= 0)
  ) %>%
...
keep_deviated <- site_cri90_male_FXL %>%
  filter(deviated_90) %>%
  pull(site_id)
...
site_cri_plot <- site_cri90_male_FXL %>%
...
grid_scaled_base <- as_tibble(grid_geo) %>%
  ...
  mutate(
    longitude_scaled = (Longitude - male_FXL_scaling$longitude["center"]) / male_FXL_scaling$longitude["scale"],
    latitude_scaled  = (Latitude - male_FXL_scaling$latitude["center"]) / male_FXL_scaling$latitude["scale"],
    mintemp_scaled   = (Mintemp - male_FXL_scaling$Mintemp["center"]) / male_FXL_scaling$Mintemp["scale"],
    maxtemp_scaled   = (Maxtemp - male_FXL_scaling$Maxtemp["center"]) / male_FXL_scaling$Maxtemp["scale"],
    minprecip_scaled = (Minprecip - male_FXL_scaling$Minprecip["center"]) / male_FXL_scaling$Minprecip["scale"],
    maxprecip_scaled = (Maxprecip - male_FXL_scaling$Maxprecip["center"]) / male_FXL_scaling$Maxprecip["scale"],
    altitude_scaled  = (Altitude - male_FXL_scaling$Altitude["center"]) / male_FXL_scaling$Altitude["scale"]
  ) %>%
  filter(
    !if_any(
      c(
...
pred_csv <- file.path(out_dir, "male_FXL_grid_epred_7periods_median_CrI90.csv")
