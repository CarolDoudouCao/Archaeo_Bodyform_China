...
```text
├── data/
│   ├── archaeological_metadata.csv                # Site metadata: group names, periods, reference
...
> The scripts are applicable to both sexes and all traits—simply change the predicted variable (e.g., from male femur length to female crural index). Additional scripts are therefore omitted to avoid duplication.
